    ...
    static public class SmartUtil
    {
        ...
        public static DateTime GetFirstDayOfWeek(this DateTime dt)
        {
            while (dt.DayOfWeek != DayOfWeek.Monday)
            {
                ...
                {//1月1号默认为该年第一周的开始,如：2012年1月1日为周日，那2012年第一周就一天
                    break;
                }
                dt = dt.AddDays(-1);
            }
        ...
        public static string GetHTMLString(System.Data.DataTable dt)
        {
            ...
            {
                string getId = SmartUtil.IngoreNull(getRow["Id"]);
                string getText = SmartUtil.IngoreNull(getRow["Text"]);
                if (index++ > 0)
                {
                    htmlString.Append(",");
                }
        ...
        {
            string temp = SmartUtil.FormatZeroCodeString(n, 2);
            if (halfTime == false)
            {
                temp += ":00 ~ " + temp + ":30";
            }
            ...
            {
                string addtemp = SmartUtil.FormatZeroCodeString(n + 1, 2);
                if (n >= 23)
                {
                    addtemp = "00";
                }
        ...
        public static string ParsePkFieldName(Type type)
        {
            ...
            List<MethodInfo> ary_Methods = SmartHtml.ParseTypeMethods(type);
        ...
        internal static string ParseSelectSql(string dbname)
        {
            ...
            List<MethodInfo> ary_Methods = SmartHtml.ParseTypeMethods(getType);
        ...
        /// <returns></returns>
        public static string EncodingString(string str, Encoding encodeType)
        {
        ...
        /// <param name="str">字符串</param>
        /// <param name="encodeType">编码类型</param>
        /// <param name="discarded">编码字节数</param>
        /// <returns></returns>
        public static string GetEncodingString(string str, Encoding encodeType, out int discarded)
        {
            discarded = 0;
            string newString = "";
            char c;
            ...
            string relStr = encodeType.GetString(bytes);
            return relStr;
        }
